...
function attachPlotHtml(pairwiseCorrelationDataAjaxRequestBody) {
	...
	var plotDivId = pairwiseCorrelationDataAjaxRequestBody.inputGene + '-'
			+ pairwiseCorrelationDataAjaxRequestBody.targetGene + '-graph';
	var tableIdExtender = pairwiseCorrelationDataAjaxRequestBody.inputGene
			+ '-' + pairwiseCorrelationDataAjaxRequestBody.targetGene;

	var expressionStageMapping = '<button type="button" class="btn btn-primary expression-btn" data-toggle="modal" data-target="#exampleModalLong">'
			+ 'See Stage Mapping' + '</button>';
	 ...
	 */
	return '<div id="' + plotDivId + '" class ="custom-plotly-plot"></div>'
			+ '<div class="text-center">' + normalizationButtonHtml
			+ expressionStageMapping + '</div>';
}
...
$(document).on(
		...
		'.normalization-btn',
		function() {
			normalizePlotData($(event.target).parents().children(
					".custom-plotly-plot")[0].id, event.target); // TODO:
			// Find
			// a
			// better
			// way
...
function normalizePlotData(plotDivId, eventTarget) {
	...
	if (normalizedPlotResults.length == 0
			|| normalizedPlotResults[0].isNormalized == false) {
		/*
		...
		for (var i = 0; i < inputGeneData.length; i++) {
			newInputGeneData.push(normalizer(inputGeneData[i], inputMax,
					inputMin));
		}
		...
		for (var i = 0; i < targetGeneData.length; i++) {
			newTargetGeneData.push(normalizer(targetGeneData[i], targetMax,
					targetMin));
		}
		...
		Plotly.restyle(plotDiv, dataUpdate, [ 1, 0 ])

		var titleUpdate = {
			title : 'Correlation Line Graph (Normalized)'
		}
		Plotly.relayout(plotDiv, titleUpdate);
		 ...
		 */
		var dataUpdate = {
			y : [ normalizedPlotResults[0].originalData[0],
					normalizedPlotResults[0].originalData[1] ],
		};
		Plotly.restyle(plotDiv, dataUpdate, [ 1, 0 ])
...
function retrieveCorrelationData(pairwiseCorrelationDataAjaxRequestBody) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			attachPlotGraphic(pairwiseGeneCorrelationData,
					pairwiseCorrelationDataAjaxRequestBody)
		},
		error : function(e) {
			console.log(e);
...
function attachPlotGraphic(pairwiseGeneCorrelationData,
		pairwiseCorrelationDataAjaxRequestBody) {
	var plotDivId = pairwiseCorrelationDataAjaxRequestBody.inputGene + '-'
			+ pairwiseCorrelationDataAjaxRequestBody.targetGene + '-graph';
	...
	var correlationData = [
			{
				name : pairwiseCorrelationDataAjaxRequestBody.inputGeneName
						&& pairwiseCorrelationDataAjaxRequestBody.inputGeneName.length < 21 ? pairwiseCorrelationDataAjaxRequestBody.inputGeneName
						: pairwiseCorrelationDataAjaxRequestBody.inputGene,
			...
			{
				name : pairwiseCorrelationDataAjaxRequestBody.targetGeneName
						&& pairwiseCorrelationDataAjaxRequestBody.targetGeneName.length < 21 ? pairwiseCorrelationDataAjaxRequestBody.targetGeneName
						: pairwiseCorrelationDataAjaxRequestBody.targetGene,
...
$('[id$="-results"] tbody')
		.on(
				...
				function() {
					...
					/*
					 * TODO: Adhoc/hard-coded way of determining which table
					 * this event belongs to. Make more dynamic
					 */
					var row;
					...
					if (row.child.isShown()) {
						...
						var pairwiseCorrelationDataAjaxRequestBody = {
							...
							inputGeneName : finalResponseCorrelationResult.inputGene.geneName,
							targetGeneName : row.data()[4],
						...
						row
								.child(
										attachPlotHtml(pairwiseCorrelationDataAjaxRequestBody))
								.show();
...
finalResponseCorrelationResult.correlationResultsForGenesOfInterest
		.sort(function(a, b) {
			if (a.corrResult.rVal < b.corrResult.rVal) {
				return 1;
			} else if (a.corrResult.rVal > b.corrResult.rVal) {
...
var rank = 1;
finalResponseCorrelationResult.correlationResults
		.forEach(function(correlationResult) {

			var buttonHtml = "";
			dataToggle = 'datatoggle="tooltip" data-placement="top" title="FlyMine '
					+ correlationResult.gene.dbIdentifier + ' Report"';
			buttonHtml += '<button type="button" class="btn btn-primary flymine-button" value="'
					+ correlationResult.gene.dbIdentifier
					+ '"'
					+ dataToggle
					+ '>FlyMine</button>';
			...
			t.row.add(
					[ null, correlationResult.rank,
							correlationResult.gene.dbIdentifier,
							correlationResult.gene.secondaryIdentifier,
							correlationResult.gene.geneName,
							correlationResult.corrResult.rValueString,
							correlationResult.corrResult.pValSigFigs,
							buttonHtml ]).draw(false);
			rank++;
		});
...
$('.flymine-button')
		.on(
				...
				function() {
					...
					var flymineUrl = 'http://www.flymine.org/query/portal.do?externalid='
							+ geneDbId
							+ '&class=Gene&origin=fly-tran-webapp&organism=D.%20melanogaster'
